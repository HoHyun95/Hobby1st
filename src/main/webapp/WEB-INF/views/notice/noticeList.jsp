				...
				<table class="w-full">
					<thead>
						<tr class="text-gray-800 border border-b-0 text-center">
							<th colspan="6" class="px-4 py-3" style="width: 100%;"># 공지사항
						</tr>
						...
						<c:forEach var="dto" items="${noticeList }" varStatus="status">
							<tr
								...
								<td class="px-4 py-4 text-center">${totalNoticeCount - ((cpage-1) * 10) - status.index}</td>
										...
										<td class="title px-4 py-4" style="cursor: pointer;"  onclick="location.href='/notice/noticeDetail?cb_seq=${dto.not_seq }&check_num=${check_num }&cpage=${cpage }&keyword=${keyword }&searchWord=${searchWord}'">
											${dto.not_title }
											
										</td>

								<td class="px-4 py-4 text-center">${dto.formDate }</td>
								<td class="px-4 py-4 text-center">${dto.not_view_count }</td>
