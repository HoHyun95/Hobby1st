...
import kh.hobby1st.dao.NoticeAdminDAO;
...
public class NoticeAdminService {
	...
	public int getRecordCount() {
		return nadao.countNotice();
	}
	
	public List<NoticeDTO> selectBoardByPaging(int cpage ) throws Exception {
		...
		return nadao.NoticeByPaging(start, end);
	}
...
// 페이지 네비게이터
	public String getPageNavi(int cpage) throws Exception {
		...
		if (needPrev) {
			pageNavi += "<a class='paging' href='/admin/noticeAdmin?cpage=" + (startNavi - 1) + "'><</a>";
		}

		for (int i = startNavi; i <= endNavi; i++) {
			pageNavi += "<a class='paging' id='paging" + i + "'" + " href='/admin/noticeAdmin?cpage=" + i + "'>" + i
					+ "</a> ";
		}

		if (needNext) {
			pageNavi += "<a class='paging' href='/admin/noticeAdmin?cpage=" + (endNavi + 1) + "'>></a>";
		}
		return pageNavi;
	}
